...
static inline void at32_setclkcr(uint32_t clkcr)
{
  ...
  regval &= ~(SDIO_CLKCR_CLKDIV89_MASK | SDIO_CLKCR_CLKDIV_MASK | SDIO_CLKCR_PWRSAV |
  ...
  clkcr  &=  (SDIO_CLKCR_CLKDIV89_MASK | SDIO_CLKCR_CLKDIV_MASK | SDIO_CLKCR_PWRSAV |
              SDIO_CLKCR_BYPASS | SDIO_CLKCR_WIDBUS_MASK |
              SDIO_CLKCR_NEGEDGE | SDIO_CLKCR_HWFC_EN |
              SDIO_CLKCR_CLKEN);
...
static void at32_dataconfig(uint32_t timeout, uint32_t dlen, uint32_t dctrl)
{
  uint32_t clkdiv;
  uint32_t regval;
  uint32_t sdio_clk = 288000000ul;//IP_CLCK_FREQ;
